...
class Register extends DBCredentials
{
	...
	public function addDetails($rollno,$name,$mail,$pwd,$role){
		...
		if($this->myerr==0 && !empty($this->myconn)){
		  ...
		  $sqlqry = "INSERT INTO `details`(`rollno`,`name`,`mail`,`pwd`,`role`) VALUES (?,?,?,?,?)";
			if ($stmt = $this->myconn->prepare($sqlqry)) {
				$stmt->bind_param("sssss",$rollno,$name,$mail,$pwd,$role);
				if($stmt->execute()){
